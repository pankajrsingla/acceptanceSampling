...
CreateVariablePlan <- function(jaspResults, dataset = NULL, options, ...) {
  ...
  # Common dependencies
  # varContainer$dependOn(c(plan_vars_single, pd_vars_single))
  ...
  # Error checking
  if (N < n) {
    if (is.null(jaspResults[["plan_error"]])) {
      plan_error <- createJaspTable(title = "", dependencies = c(risk_vars, pd_vars), position = 3)
      plan_error$setError(sprintf("Error: Invalid input. Lot size (N) cannot be smaller than the sample size (n)."))
      jaspResults[["plan_error"]] <- plan_error
      return ()
    }
  }
  ...
  .variablePlanTable(jaspResults, sd, risk_vars, n, k, pos=5)
  ...
  if (is.null(jaspResults[["decision_info"]])) {
    plan_op <- createJaspHtml(text = sprintf("%s\n\n%s", "Z.LSL = (mean - LSL) / historical standard deviation", "Accept lot if Z.LSL >= k, otherwise reject."), 
                              dependencies = c(risk_vars, "sd"), position = 4)
    jaspResults[["decision_info"]] <- plan_op
  }

  # 1. Plan summary
  if (options$showSummary) {
    getSummary(jaspResults, pos=6, c(risk_vars, pd_vars, output_vars[1]), df_plan)
  }
  # 2. OC Curve
  if (options$showOCCurve) {
    getOCCurve(jaspResults, pos=7, c(risk_vars, pd_vars, output_vars[2]), df_plan)
  }
  # 3. AOQ Curve
