...
.assessPlanSingle <- function(jaspResults, options) {
  ...
  depends_risk_table <- c("lotSizeSingle", "sampleSizeSingle", "acceptNumberSingle", "rejectNumberSingle", "distributionSingle", "pd_prpSingle", "pa_prpSingle", "pd_crpSingle", "pa_crpSingle")
  table2 <- getRiskPointTable(jaspResults, depends_risk_table, assess, options$pd_prpSingle, options$pa_prpSingle, options$pd_crpSingle, options$pa_crpSingle)

  # OC Curve
  if (options$showOCCurveSingle) {
    depends_oc <- c(depends_risk_table[1:5], "showOCCurveSingle")
    ocPlot_single <- getPlot(jaspResults, df_x, depends_oc, "single")
  }

  # Plan summary
  if (options$showSummarySingle) {
    depends_summary <- c(depends_risk_table[1:5], "showSummarySingle")
    printSummary(jaspResults, df_x, depends_summary)
  }
}
...
.assessPlanMult <- function(jaspResults, options) {
  ...
  # Get plan
  output <- getPlanDf(options$sampleSizeMult, options$acceptNumberMult, options$rejectNumberMult, dist_mult, options$lotSizeMult, TRUE)
  x_mult <- output[[1]]
  df_x_mult <- output[[2]]
  ...
  # 2. Table with the specified and actual acceptance probabilities
  depends_risk_table <- c("lotSizeMult", "sampleSizeMult", "acceptNumberMult", "rejectNumberMult", "distributionMult", "pd_prpMult", "pa_prpMult", "pd_crpMult", "pa_crpMult")
  table2 <- getRiskPointTable(jaspResults, depends_risk_table, assess_mult, options$pd_prpMult, options$pa_prpMult, options$pd_crpMult, options$pa_crpMult)
  
  # OC Curve
  if (options$showOCCurveMult) {
    depends_oc <- c(depends_risk_table[1:5], "showOCCurveMult")
    ocPlot_mult <- getPlot(jaspResults, df_x_mult, depends_oc, "Mult")
  }

  # ASN Curve
  if (options$showASNCurve) {
    depends_asn <- c(depends_risk_table[1:5], "showASNCurveMult")
    asnCurve <- getASNCurve(jaspResults, options, depends_asn)
  }

  # Plan summary
  if (options$showSummaryMult) {
    depends_summary <- c(depends_risk_table[1:5], "showSummaryMult")
    printSummary(jaspResults, df_x_mult, depends_summary)
  }
}